...
const refreshLastCalledFunction = () => {
  if (lastCalledFunction == getItemsByOrderStatus) {
    console.log("Function: Get all items");
  } else {
    console.log("Function: " + lastCalledFunction);
  }
  lastCalledFunction();
};
...
let lastEnteredId = localStorage.getItem("lastEnteredId");
...
// Call getPartsById() with the last entered ID on page load
if (lastEnteredId) {
  getPartsById(lastEnteredId);
}

// Call getPartsById() every 15 seconds with the last entered ID
setInterval(() => {
  if (lastEnteredId) {
    getPartsById(lastEnteredId);
    console.log("Refreshed part of item: " + lastEnteredId);
  }
}, 15000);
